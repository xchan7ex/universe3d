...
function readGlbBounds(filePath) {
    ...
    // Three.js Box3 combines all geometries. We can find the min/max of the POSITION accessors.
    ...
    if (json.accessors) {
        json.accessors.forEach(acc => {
            if (acc.type === 'VEC3' && acc.min && acc.max) {
                // Heuristic: accumulate bounding box over all VEC3 accessors (typically POSITION)
